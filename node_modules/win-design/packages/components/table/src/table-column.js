...
const defaults = {
  ...
  selection: {
    ...
    realWidth: 48,
    order: '',
    className: 'w-table-column--selection'
  },
  expand: {
    width: 48,
    minWidth: 48,
...
const forced = {
  ...
  expand: {
    ...
    renderCell: function (h, { row, store }, proxy) {
      ...
      return <div class={ 'w-table__expand-icon ' + (expanded ? 'w-table__expand-icon--expanded' : '') }
        on-click={ e => proxy.handleExpandClick(row, e) }>
        <i class='w-icon w-icon-arrow-right'></i>
      </div>
    },
    sortable: false,
    resizable: false,
    className: 'w-table__expand-column'
  }
}
...
export default {
  ...
  created () {
      ...
      if (value !== undefined) {
        if (prop === 'renderHeader') {
          if (type === 'selection' && column[prop]) {
            console.warn('[Win-Design Warn][TableColumn]Selection column doesn\'t allow to set render-header function.')
          } else {
    ...
    if (this.renderHeader) {
      console.warn('[Win-Design Warn][TableColumn]Comparing to render-header, scoped-slot header is easier to use. We recommend users to use scoped-slot header.')
    }
    ...
    column.renderCell = function (h, data) {
      ...
      return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
        ? <div class="cell is-overflow" style={ {width: (data.column.realWidth || data.column.width) - 1 + 'px'} }>{ renderCell(h, data) }</div>
  ...
  mounted () {
    ...
    if (this.$scopedSlots.header) {
      if (this.type === 'selection') {
        console.warn('[Win-Design Warn][TableColumn]Selection column doesn\'t allow to set scoped-slot header.')
      } else {
